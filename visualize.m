...
ColorMap	= fread(fopen(['Fig_nx',num2str(nx),'_',num2str(Size),'km_ColorMap.bin'],'r'),[nx,nx],'double');
Typography	= fread(fopen(['Fig_nx',num2str(nx),'_',num2str(Size),'km_Typography.bin'],'r'),[nx,nx],'double');
...
filename = ['Solution_nx',num2str(nx),'_',num2str(Size),'km_T',num2str(Tend),'_h.bin'];
if exist(filename, 'file') == 2
...
% Set color axis
load('cmap.mat');colormap(cmap);caxis([-10 10]);
